...
const [poseDetected, setPoseDetected] =
  ...
  const stopWorkout = async () => {
    const confirmed = window.confirm("ต้องการหยุด Workout หรือไม่?");
    if (!confirmed) return;

    try {
      const resultScore = Number(aiResult?.score ?? poseAnalysis?.score ?? 0);
      await api.post('/workouts', {
        exerciseId: selectedExercise.id,
        repetitions: Number(aiResult?.reps ?? 0),
        duration,
        score: Math.max(0, Math.min(100, resultScore)),
        startedAt,
        completedAt: new Date().toISOString(),
      });
      setError('บันทึก Workout สำเร็จ ✓');
    } catch (error) {
      setError(error.response?.data?.message || error.message || 'ไม่สามารถบันทึก Workout ได้');
    } finally {
